...
public class AdditionFactory implements Expression.ExpressionFactory {
  ...
  public Expression of(String expression) {
    ...
    for (int i = expression.length() - 1; i >= 0; i--) {
      if (expression.charAt(i) == ')') {
        ...
        return new Addition(resultOfExpression(expression.substring(0, i)), resultOfExpression(expression.substring(i + 1)));
      }
    }
    throw new IllegalArgumentException();
  }

  private static class Addition implements Expression {
